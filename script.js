...
function switchSection(portion,section) {
	if (portion == 0 && sections[portion] != section) {
		if (section == 1) {
			...
			document.getElementById("beforePodreelSelection").style.backgroundColor = "white";
			document.getElementById("afterPodreelSelection").style.backgroundColor = "transparent";
			...
			document.getElementById("afterPodreelSelection").style.color = "rgba(255,255,255,.6)";
		}
		if (section == 2) {
			sections[portion] = section;
			document.getElementById("beforePodreelSelection").style.backgroundColor = "transparent";
			document.getElementById("afterPodreelSelection").style.backgroundColor = "white";
			...
			document.getElementById("beforePodreelSelection").style.color = "rgba(255,255,255,.6)";
		}
	}else if (portion == 1 && sections[portion] != section) {
		if (section == 1) {
			sections[portion] = section;
			document.getElementById("audioPodcasts").style.backgroundColor = "white";
			document.getElementById("videoPodcasts").style.backgroundColor = "transparent";
			...
			document.getElementById("videoPodcasts").style.color = "rgba(255,255,255,.6)";
		}
		if (section == 2) {
			sections[portion] = section;
			document.getElementById("audioPodcasts").style.backgroundColor = "transparent";
			...
			document.getElementById("audioPodcasts").style.color = "rgba(255,255,255,.6)";
		}
	}
}
...
class Star {
  ...
  reset = () => {
    ...
    const travelled =
      Math.random() > 0.5
        ? Math.random() * Math.max(window.innerWidth, document.getElementById("container").offsetHeight) + (Math.random() * (window.innerWidth * 0.24))
    ...
    this.STATE = {
      ...
      vX,
      y: Math.floor(vY * travelled) + document.getElementById("container").offsetHeight / 2,
      vY,
      size: BASE_SIZE,
    }
  }
...
class JumpToHyperspace {
  ...
  render = () => {
    ...
    // Clear the canvas
    context.clearRect(0, 0, window.innerWidth, document.getElementById("container").offsetHeight)
    if (bgAlpha > 0) {
      context.fillStyle = `rgba(31, 58, 157, ${bgAlpha})`
      context.fillRect(0, 0, window.innerWidth, document.getElementById("container").offsetHeight)
    }
